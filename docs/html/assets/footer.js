...
(function () {
  ...
  var css = `
    ...
    .foot-nav .fn-admin{ color:#C6C6C3; }   /* 관리자 — 한 단계 더 흐리게 (reg-detail과 동일 톤). backend 진입점(네이버 로그인 예정) */
    ...
    /* 고정(fixed) 아님: 문서 흐름 맨 아래(푸터 뒤)에 놓여 스크롤해야 보임 */
    .foot-marquee{ background:#141414; overflow:hidden; }
    .foot-marquee-inner{ max-width:1200px; margin:0 auto; padding:0 24px; display:flex; align-items:center; height:38px; }   /* 좌우 24px = .wrap과 동일 (상단 속보 티커와 통일) */
    .foot-marquee-label{ flex:0 0 auto; font-family:var(--serif); font-size:13px; font-weight:700; line-height:1.6; letter-spacing:normal; color:var(--orange); padding-right:16px; margin-right:0; border-right:1px solid rgba(255,255,255,.18); }   /* 상단 속보 라벨과 동일 규격(색상만 유지) · margin 0 → 글씨가 짝대기에 닿으며 사라짐 */
    .foot-marquee-viewport{ flex:1 1 auto; overflow:hidden; }
    /* 마퀴 오른쪽 끝 짝대기 — 왼쪽(라벨 border)과 동일 규격·높이 */
    .foot-marquee-end{ flex:0 0 auto; width:0; height:calc(13px * 1.6); border-left:1px solid rgba(255,255,255,.18); }
    .foot-marquee-track{ display:flex; align-items:center; width:max-content; animation:foot-marquee-scroll 66s linear infinite; }   /* 44s→66s 더 천천히 */
    ...
    @media (max-width:640px){ .foot-marquee-inner{ padding:0 18px; } }   /* .wrap 모바일 패딩(18px)과 동일 정렬 */
  `;
    ...
    '<div class="foot-marquee">' +
      '<div class="foot-marquee-inner">' +
        ...
        '<div class="foot-marquee-viewport">' +
          ...
          '</div>' +
        '</div>' +
        '<span class="foot-marquee-end" aria-hidden="true"></span>' +
      '</div>' +
    '</div>';
