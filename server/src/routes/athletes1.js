// 선수(athletes1) — /api/athletes 의 작업 복사본. 기존 athletes 는 그대로 두고,
// 여기서 '기계적 규칙(코드)으로 대부분 생성 + 어려운 부분만 LLM' 하이브리드 기사 생성을 실험한다.
// 마운트: /api/athletes1 (app.js). 초기 내용은 athletes.js 와 동일.
...
// 기사 생성 (하이브리드) — 숫자·기록·통계·표·태그·미디어는 코드로 결정적 조립(환각 0),
// title·subtitle·lead·대회소개(①)·선수요약(②)만 LLM. 전체 DB 문서(JSON)를 반환.
router.post('/generate-article', async (req, res) => {
  try {
    const data = (req.body && req.body.data) || req.body || {}
    const name = data?.athlete?.name || data?.name
    if (!name) return res.status(400).json({ error: '선수 데이터가 없습니다.' })
    // 판단 필드만 생성하므로 토큰이 작다(잘림 없음). 제공자는 .env LLM_MODEL.
    const maxTokens = Number(process.env.LLM_MAX_TOKENS) || 1500
    const { doc, meta } = await buildHybridArticle(data, { temperature: 0.4, maxTokens })
    res.json({ content: JSON.stringify(doc, null, 2), finishReason: meta.finishReason, usage: meta.usage, provider: meta.provider, model: meta.model, meta })
  } catch (e) {
